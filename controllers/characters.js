const characterService = require("../services/characters"),
  characterUtilities = require("../utils/character");
...
exports.listCharacters = async (req, res) => {
  let characters = await characterService.fetchCharactersInMovie(
    req.params.movieId
  );
  ...
  if ("sortBy" in req.query) {
    if (
      ...
      req.query.sortBy !== "gender" &&
      req.query.sortBy !== "height"
    ) {
    ...
    characters = await characterUtilities.sortCharacters(
      characters,
      req.query.sortBy,
      req.query.order
    );
  ...
  if ("gender" in req.query) {
    ...
    characters = await characterUtilities.filterCharacterByGender(
      characters,
      req.query.gender
    );
  }

  characters = await characterUtilities.getTotalHeightAndCharacterCount(
    characters
  );
